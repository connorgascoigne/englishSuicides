...
data.model <-
  data.final %>%
  # grouping columns for covariates (labels)
  dplyr::mutate(deprivationGroup = imdScore %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE),
                diversityGroup = nonWhite %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE),
                populationDensityGroup = populationDensity %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE),
                nighttimeLightGroup = nighttimeLight %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE),
                totalRailGroup = dplyr::if_else(totalRail == 0, 1, 2) %>% as.factor(),
                totalRoadGroup = totalRoad %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE),
                ndviGroup = ndvi %>% cut(x = ., breaks = stats::quantile(x = ., probs = 0:n.groups/n.groups), include.lowest = TRUE)) %>%
