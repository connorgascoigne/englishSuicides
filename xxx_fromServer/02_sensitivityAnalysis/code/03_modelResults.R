...
theta.predicted <-
  ...
  dplyr::bind_cols(.,
                   data.model) %>% 
  # sort some of the data columns
  dplyr::mutate(dplyr::across(dplyr::ends_with('Group'), ~ .x %>% as.factor() %>% as.numeric())) %>% 
  # send theta to back 
...
#              name.label = c('Deprivation', 'Ethnic Density', 'Population Density', 'Light Pollution', 'Railway Network', 'Road Network Density', 'Greenspace'),
#              reference = c('Ref. 1 (Least Deprived)', 'Ref. 1 (Least Dense)', 'Ref. 1 (Least Dense)', 'Ref. 1 (Least Polluted)', 'Ref. 1 (No Railway)', 'Ref. 1 (Least Dense)', 'Ref. 1 (Least Green)'),
#              max = c(paste0(n.groups, ' (Most Deprived)'),
#                      paste0(n.groups, ' (Most Dense)'),
#                      paste0(n.groups, ' (Most Dense)'),
#                      paste0(n.groups, ' (Most Polluted)'),
#                      '2 (Some Railway)',
#                      paste0(n.groups, ' (Most Dense)'),
#                      paste0(n.groups, ' (Most Green)')))
